...
class ChunkedGenerator:
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, valid_frame,
                 ...
                 endless=False, out_all = False, MAE=False, train=True, tds=1):
        ...
        self.train=train
        print('3dhp length: ', len(self.pairs))
    ...
    def get_batch(self, seq_i, start_3d, end_3d, flip, reverse):
        ...
        if self.train == True:
            if self.MAE:
                return np.zeros(9), self.batch_2d.copy(), self.batch_3d.copy(), seq, subject, int(cam_index)
            ...
            if self.MAE:
                return np.zeros(9), self.batch_2d.copy(),self.batch_3d.copy(), seq_name, None, None
            else:
                return np.zeros(9), self.batch_3d.copy(), self.batch_2d.copy(), seq_name, None, None

  
    def get_batch2(self, seq_i, start_3d, end_3d, flip, reverse):
        if self.train==True:
            subject,seq,cam_index = seq_i
            seq_name = (subject,seq,cam_index)
        else:
        ...
        if pad_left_2d != 0 or pad_right_2d != 0:
            self.batch_2d = np.pad(seq_2d[low_2d:high_2d], ((pad_left_2d, pad_right_2d), (0, 0), (0, 0)), 'edge')
            self.batch_2d = self.batch_2d[::self.tds]
        else:
        ...
        if not self.MAE:
            if self.poses_3d is not None:
                ...
                if pad_left_3d != 0 or pad_right_3d != 0:
                    self.batch_3d = np.pad(seq_3d[low_3d:high_3d],
                                              ((pad_left_3d, pad_right_3d), (0, 0), (0, 0)), 'edge')
                    self.batch_3d = self.batch_3d[::self.tds]
                else:
