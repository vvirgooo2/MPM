...
def wmpjpe_cal(predicted, target,w):
    """
...
def test_calculation(predicted, target, action, error_sum, data_type, subject, MAE=False):
    error_sum, detail = mpjpe_by_action_p1(predicted, target, action, error_sum)
    if not MAE:
        error_sum = mpjpe_by_action_p2(predicted, target, action, error_sum)

    return error_sum, detail
...
def mpjpe_by_action_p1(predicted, target, action, action_error_sum):
    ...
    detail = torch.norm(predicted - target, dim=len(target.shape) - 1)
    detail = detail.permute(2,0,1)
    detail = torch.mean(detail,dim=1)
    detail = torch.mean(detail,dim=1)
    detail = detail.detach().cpu().numpy()
    ...
    if len(set(list(action))) == 1:
        ...
        # print(action_error_sum[action_name])
    ...
    return action_error_sum, detail
...
def save_model(previous_name, save_dir,epoch, data_threshold, model, model_name,opt):
    # if os.path.exists(previous_name):
    #     previous_name = previous_name + 'a'

    torch.save(model.state_dict(),
               '%s/%s_%s_%d_%d_%dD.pth' % (save_dir,opt.name, model_name, epoch, data_threshold * 100,opt.dim))

    previous_name = '%s/%s_%s_%d_%d_%dD.pth' % (save_dir,opt.name, model_name, epoch, data_threshold * 100,opt.dim)
...
def set_grad(nets, requires_grad=False):
    " set params in nets requires_grad (params outside nets will be frozen)"
    for net in nets:
        for param in net.parameters():
            param.requires_grad = requires_grad
            
def compute_PCK(gts, preds, scales=1000, eval_joints=None, threshold=0.15):
    PCK_THRESHOLD = threshold
    sample_num = len(gts)
    sample_frame = gts.shape[1]
    total = 0
    true_positive = 0
    if eval_joints is None:
        eval_joints = list(range(gts.shape[2]))

    for n in range(sample_num):
        for f in range(sample_frame):
            gt = gts[n][f]
            pred = preds[n][f]

            per_joint_error = np.take(np.sqrt(np.sum(np.power(pred.cpu().numpy() - gt.cpu().numpy(), 2), 1)), eval_joints, axis=0)
