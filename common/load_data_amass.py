...
class Fusion(data.Dataset):
    ...
    def __getitem__(self, index):
        assert self.opt.shuffle ==1, "3dhp don't support this task"
        assert self.opt.MAE ==1, "3dhp don't support 3dhpe task"
        ...
        if flag3d==1:
            return  np.zeros((0,16,2)), matched_3D, np.zeros(0,)
        else:
            return matched_2D, np.zeros((0,16,3)), np.zeros(0,)
