...
def get_loader(opt):
    ...
    if opt.MAE:
        ...
        if opt.train:
            ...
            train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=opt.batchSize,shuffle=True, num_workers=int(opt.workers), pin_memory=True,collate_fn=mycollate)
        if opt.test:
            test_data = HybirdSet(opt,opt.dataset,False,hm36dataset=dataset)
            test_dataloader = torch.utils.data.DataLoader(test_data , batch_size=opt.batchSize,shuffle=False, num_workers=int(opt.workers), pin_memory=True,collate_fn=mycollate)
    else:
        if opt.dataset[0]=='h36m':
            print('11111111')
            dataset_path = root_path + 'data_3d_' + 'h36m' + '.npz'
            dataset = Human36mDataset(dataset_path,opt)
