...
from Augpart.gan_utils import get_bone_lengthbypose3d, get_bone_unit_vecbypose3d, \
...
class BAGenerator(nn.Module):
    ...
    def forward(self, inputs_3d):
        ...
        # pre-processing
        noise = torch.randn(sx.shape[0], self.noise_channle, device=x.device)
        y = self.w1(torch.cat((sx, noise), dim=1))
        y = self.batch_norm1(y)
        y = self.relu(y)
        ...
        y = self.w2(y)
        y = y.view(sx.size(0), -1, 3) 
        
        # repeat to be sequence 
        y = torch.unsqueeze(y, dim = 1)
        y = y.repeat(1,f,1,1).view(b*f, -1, 3)
        ...
        out = get_pose3dbyBoneVec(modifyed_bone).view(b,f,j,n) + root_origin
...
class RTGenerator(nn.Module):
    ...
    def forward(self, inputs_3d, augx):
        ...
        # caculate R
        noise = torch.randn(sx.shape[0], self.noise_channle, device=x.device)
        r = self.w1_R(torch.cat((sx, noise), dim=1))
        ...
        r = self.w2_R(r)
        r = nn.Tanh()(r) * 3.1415
        r = r.view(sx.size(0), 3)
        r = torch.unsqueeze(r, dim = 1)
        r = r.repeat(1,f,1).view(b*f, 3)
        rM = tgm.angle_axis_to_rotation_matrix(r)[..., :3, :3]  # Nx4x4->Nx3x3 rotation matrix

        # caculate T
        noise = torch.randn(sx.shape[0], self.noise_channle, device=x.device)
        t = self.w1_T(torch.cat((sx, noise), dim=1))
        ...
        t = t.view(sx.size(0), 1, 3)  # Nx1x3 translation t
        t = torch.unsqueeze(t, dim = 1)
        t = t.repeat(1, f, 1, 1).view(b*f, 1, 3)
        
        # operat RT on original data - augx
        augx = augx - augx[:, :,  :1, :]  # x: root relative
        augx = augx.view(b*f, j, n).permute(0, 2, 1).contiguous()
        augx_r = torch.matmul(rM, augx)
        augx_r = augx_r.permute(0, 2, 1).contiguous()
        augx_rt = augx_r + t

        augx_rt = augx_rt.view(b,f,j,n)
...
class BLGenerator(nn.Module):
    ...
    def forward(self, inputs_3d, augx):
        ...
        # caculate blr
        bones_length_x = get_bone_lengthbypose3d(sx.view(sx.size(0), -1, 3)).squeeze(2)  # 0907
        noise = torch.randn(sx.shape[0], self.noise_channle, device=x.device)
        blr = self.w1_BL(torch.cat((sx, bones_length_x, noise), dim=1))
        ...
        # blr (b, 9) repeat for f
        blr = torch.unsqueeze(blr, dim = 1)
        blr = blr.repeat(1, f, 1).view(b*f, 9)
...
def random_bl_aug(x):
    ...
    :return: nx16x3
    '''
    bl_15segs_templates_mdifyed = np.load('./data/bone_length_npy/hm36s15678_bl_templates.npy')
