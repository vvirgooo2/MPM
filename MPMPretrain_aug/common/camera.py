...
def camera2pixel(points_3d, cam): 
    """ 将相机坐标系下的3D点位置转换为像素坐标系下的3D点位置。

    Args:
        points_3d: 相机坐标系下的3D点位置，形状为 (B, N, 3)，其中 B 为 batch size，N 为点的数量。
        cam: 定义相机内参矩阵和分辨率大小的字典
            {
                'focal_length': np.array([fx, fy], dtype=float), 
    ...
    Returns:
        返回像素坐标系下的3D点位置，形状为 (B, N, 3)，其中 B 为 batch size，N 为点的数量。
    """
    assert points_3d.shape[-1] == 3, 'points_3d should have shape (B, N, 3).'
    fx, fy = cam['focal_length']
    cx, cy = cam['center']
    bs, num_pts = points_3d.shape[:2]
    depth = points_3d[..., -1]
    xy = points_3d[..., :2] / np.tile(depth[..., np.newaxis], (1, 1, 2))
    xy[..., 0] = xy[..., 0] * fx + cx
    xy[..., 1] = xy[..., 1] * fy + cy

    return np.concatenate([xy, depth[..., np.newaxis]], axis=-1)
