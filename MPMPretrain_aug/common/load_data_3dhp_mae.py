...
class Fusion(data.Dataset):
    ...
    def __len__(self):
        ...
        if self.MAE and self.only2d:
            return int(len(self.generator.pairs) * self.rateof2d)
        elif self.MAE and self.only3d:
            return int(len(self.generator.pairs) * self.rateof3d)
        else:
    ...
    def __getitem__(self, index):
        ...
        # 预训练数据
        if self.MAE and (flag3d==0 or self.only2d):        
            if self.train == False and self.test_aug:
                _, input_2D_aug,_, _, _,_ = self.generator.get_batch(seq_name, start, end, flip=True, reverse=reverse)
                input_2D = np.concatenate((np.expand_dims(input_2D,axis=0),np.expand_dims(input_2D_aug,axis=0)),0)
                
                return input_2D,np.zeros((0,0,16,3))
            return input_2D,np.zeros((0,16,3))
        
        if self.MAE and (flag3d==1 or self.only3d):                        
            if self.train == False and self.test_aug:
                _, _,input_3D_aug, _, _,_ = self.generator.get_batch(seq_name, start, end, flip=True, reverse=reverse)
                input_3D = np.concatenate((np.expand_dims(input_3D,axis=0),np.expand_dims(input_3D_aug,axis=0)),0)
                return np.zeros((0,0,16,2)),input_3D
            return np.zeros((0,16,2)),input_3D
