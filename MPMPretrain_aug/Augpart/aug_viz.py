...
def plot_poseaug(inputs_3d, inputs_2d, g_rlt, cam_param, epoch, iter, args):
    ...
    # (b,f,17,n)
    f = outputs_2d_ba.shape[1]
    for i in range(0,f):
        # plot the augmented pose from origin -> ba -> bl -> rt
        _plot_poseaug(
            inputs_3d.cpu().detach().numpy()[0][i], inputs_2d.cpu().detach().numpy()[0][i],
            outputs_3d_ba.cpu().detach().numpy()[0][i], outputs_2d_ba.cpu().detach().numpy()[0][i],
            outputs_3d_bl.cpu().detach().numpy()[0][i], outputs_2d_bl.cpu().detach().numpy()[0][i],
            outputs_3d_rt.cpu().detach().numpy()[0][i], outputs_2d_rt.cpu().detach().numpy()[0][i],
            epoch, iter, args, i
        )
...
def _plot_poseaug(
    ...
    os.makedirs('{}/poseaug_viz/epoch_{:0>4d}_iter_{:0>4d}/'.format(args.checkpoint,epoch, iter), exist_ok=True)
    
    
    image_name = '{}/poseaug_viz/epoch_{:0>4d}_iter_{:0>4d}/{:0>4d}_frame_.png'.format(args.checkpoint, epoch, iter, frame)
